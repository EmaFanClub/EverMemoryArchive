...
- 对日程编辑做权限收窄：只允许更新摘要/正文、可编辑单次日程时间，并允许删除可编辑日程；不允许改周期、任务类型或目标会话。
- 对记忆编辑保持记录身份不变：只更新 `memory` 正文或删除整条记录，更新时保留 actor、kind、date、dayDate、processedAt 等元数据。
- 覆盖加载、空状态、保存失败、角色切换等 UI 状态。

**Non-Goals:**

- 不新增日程创建、改周期、改目标会话能力；仅支持编辑已有可编辑单次日程的时间和删除已有可编辑日程。
- 不新增长期记忆检索或长期记忆编辑 UI。
...
- `DELETE /api/v1beta1/actors/[actorId]/schedules/[scheduleId]`
- `GET /api/v1beta1/actors/[actorId]/memories`
- `PATCH /api/v1beta1/actors/[actorId]/memories/[memoryId]`
- `DELETE /api/v1beta1/actors/[actorId]/memories/[memoryId]`
...
### 2. 日程 PATCH 只接受文本字段和单次时间

WebUI 日程更新接口只接受 `summary`、`prompt` 和可编辑单次日程的 `runAt`。服务层读取当前日程后，用现有 `ActorScheduler.update()` 传入受限 payload，显式不接受 `interval`、`conversationId` 或 `task`。`runAt` 必须是有效的 `YYYY-MM-DD HH:mm:ss` 格式，且仅对单次日程生效。

WebUI 日程删除接口只允许删除同一 actor 下的可编辑日程，并复用 `ActorScheduler.delete()`。UI 删除前必须弹窗确认，避免误删。

`chat` 和 `activity` 日程通常有可编辑文本；`wake`、`sleep` 和 `focus` 缺少用户可维护文本内容时应在 UI 中只读展示。这样既满足“可编辑内容”，也避免误开时间和执行语义。

### 3. 记忆面板使用短期记忆作为数据源

用户说的“年记、月记、日记、活动”对应短期记忆 `kind: "year" | "month" | "day" | "activity"`。本需求不涉及长期记忆 `long_term_memories`。记忆 PATCH 通过 id 定位 actor-owned 短期记忆，更新正文并保留原有元数据；记忆 DELETE 删除同一 actor 下的指定短期记忆，UI 删除前必须弹窗确认。

记忆列表采用与日程列表一致的“外层简洁卡片 + 详情弹窗编辑”模式。外层卡片展示日期、正文预览和完整更新时间；活动记忆额外在左上角展示来源胶囊。当前成熟数据模型尚未持久化 activity 的会话来源，因此首版来源胶囊使用现有可表达的“活动”兜底，不读取未实现的 activity source spec。

记忆列表的返回数量应贴近当前记忆窗口：年记保留保守列表上限，月记、日记和活动记忆分别按当前 prompt 窗口大小返回，避免 UI 计数暗示超过实际窗口的上下文规模。
...
- [Risk] 日程底层 update 能修改周期和目标会话，WebUI API 若透传 payload 会绕过需求限制 -> Mitigation: route/service 层白名单校验字段，并加测试确认不支持的字段不被接受。
- [Risk] 日程底层 delete 能删除系统日程，WebUI API 若只按 id 删除会绕过界面限制 -> Mitigation: service 层删除前读取当前日程并确认 task 可编辑。
