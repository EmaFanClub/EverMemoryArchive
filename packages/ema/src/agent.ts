...
export class ContextManager {
	...
	estimateTokens(): number {
		...
		try {
			...
			console.warn(`Token estimation fallback due to error: ${(error as Error).message}`);
	...
	async summarizeMessages(): Promise<void> {
		...
		const shouldSummarize =
			estimatedTokens > this.tokenLimit || this.apiTotalTokens > this.tokenLimit;
		...
		for (let i = 0; i < userIndices.length; i += 1) {
			...
			const nextUserIdx = i < userIndices.length - 1 ? userIndices[i + 1] : this.messages.length;
	...
	async run(): Promise<string> {
        ...
        // Truncate each argument value to avoid overly long output
				...
				for (const [key, value] of Object.entries(callArgs)) {
					...
					truncatedArgs[key] = valueStr.length > 200 ? `${valueStr.slice(0, 200)}...` : value;
				}
				...
				if (result.success) {
					...
					console.log(`${Colors.BRIGHT_GREEN}✓ Result:${Colors.RESET} ${resultText}`);
				} else {
					console.log(
